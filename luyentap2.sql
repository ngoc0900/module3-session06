...
DELIMITER $$
create trigger trigger_change_counts
after update on accounts
for each row
begin
	 insert into logs_ (log_id,account_id,old_sum,new_sum) 
     values(old.account_holder_id,old.id,old.balance,new.balance);
end
$$ DELIMITER ;

update accounts set balance = 555.345 where id = 10;
select * from logs_;